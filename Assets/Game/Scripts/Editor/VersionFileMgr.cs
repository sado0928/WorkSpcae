...
namespace Game.Editor
{
    ...
    /// </summary>
    public static class VersionFileMgr
    {
        /// <summary>
        ...
        public static void GenerateFileListAndClean()
        {
            ...
            // 1. 寻找最新的 Catalog 文件
            var catalogFiles = new DirectoryInfo(serverDataPath).GetFiles("catalog_*.json")
            ...
            if (catalogData != null && catalogData.m_InternalIds != null)
            {
                foreach (var id in catalogData.m_InternalIds)
                {
                    if (id.EndsWith(".bundle"))
                    {
                        ...
                        if (fileNameLower.Contains("resbuildin") || 
                            fileNameLower.Contains("addressableassetsdata") || 
                            fileNameLower.Contains("catalog_"))
                        {
            ...
            File.WriteAllText(Path.Combine(streamingAssetsPath, "filelist.json"), JsonUtility.ToJson(builtInManifest, true));
            File.WriteAllText(Path.Combine(streamingAssetsPath, "version.txt"), versionContent); 
